...
@channel.use(
    ListenerSchema(
        ...
        decorators=[MatchContent("原神")],
    )
)
async def genshin(app: Ariadne, friend: Friend):  # lsp专属
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(pic_url) as r:
                pic = await r.read()
                await app.send_friend_message(friend, MessageChain(FlashImage(data_bytes=pic)))
